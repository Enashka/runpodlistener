...
3. Initial setup (if this is your first time):
   ```bash
   ...
   For updates (if you already have the tool installed):
   ```bash
   # Go to the existing installation directory
   cd /workspace/runpodlistener
   
   # Download the latest version of the setup script
   curl -O https://raw.githubusercontent.com/Enashka/runpodlistener/main/setup_sync.sh
   chmod +x setup_sync.sh
   ./setup_sync.sh
   ```
